...
Обновление скачивается во временный `.download` файл и запускается только после обязательной проверки SHA-256 из официального релиза. Кэшированный файл проверяется повторно, а целостность ещё раз сверяется непосредственно перед UAC. После появления цифровой подписи автоматическое обновление также потребует Authenticode-сертификат того же издателя.

Политика безопасности и порядок сообщения об уязвимостях описаны в [`SECURITY.md`](SECURITY.md).
